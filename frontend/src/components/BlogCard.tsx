...
const BlogCard = ({content} : {content : Array<{
    id : string,
    title : string,
    content : string,
    thumbnail : string,
    ...
    {
        content.map((e) => {
            return <div className="flex w-full py-[1rem] border-b-[1px] gap-[1rem] border-b-[#6B6B6B] flex-col" key={e.id}>
                {/* // avatar + username */}
                <div className="w-full flex items-center gap-[0.5rem]">
                    ...
                    style={{fontFamily: `sohne, "Helvetica Neue", Helvetica, Arial, sans-serif`}}>{e.author}</div>
                </div>
                {/* // title + content + thumbnail */}
                <div className="w-full flex justify-between" style={{fontFamily: `medium-content-sans-serif-font, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen, Ubuntu, Cantarell, "Open Sans", "Helvetica Neue", sans-serif`}}>
                    {/* // title + content */}
                    ...
                    <div className="w-[20%] h-full">
                        ...
                        <img src="https://images.pexels.com/photos/546819/pexels-photo-546819.jpeg" alt="image" className="w-full h-full object-cover" />
                    </div>
                </div>
                ...
                <div className="w-full text-[#6B6B6B] flex items-center justify-between">
                    ...
                    <div>
                        {e.publishedOn.split('T')[0]}
                    </div>
                    <div>
                        {Math.floor(e.content.length / 60)} min(s) read
                    </div>
                    </div>
